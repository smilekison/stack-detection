...
def analyze(repo,spec,result):
    ...
    if _static(repo,primary,framework):
        ...
        check('STATIC_ENTRYPOINT','Static HTML entrypoint','pass',[f for f in repo.files if Path(f).name=='index.html'][:3],'Static site detected; no application runtime is required.')
        check('STATIC_RUNTIME','Static runtime','pass',[],'Nginx will serve repository content.')
        decisions += [{'code':'TARGET','decision':'static-nginx','reason':'No server runtime or package build system detected.'},{'code':'PORT','decision':80,'reason':'Nginx default HTTP port.'}]
    elif primary in {'JavaScript','TypeScript'} or 'package.json' in repo.file_set:
        scripts=pkg.get('scripts') or {};lock=LOCKFILES.get(pm);has_lock=bool(lock and lock in repo.file_set)
        check('NODE_MANIFEST','Node package manifest','pass' if 'package.json' in repo.file_set else 'blocker',['package.json'] if 'package.json' in repo.file_set else [],'package.json required for deterministic Node builds.')
        ...
        build=scripts.get('build');start=scripts.get('start');dev=scripts.get('dev');check('BUILD_SCRIPT','Production build command','pass' if build else 'blocker',['package.json'],build or 'No build script found.')
        ...
        spec.runtime['version']=str((pkg.get('engines') or {}).get('node','20')).lstrip('v').split()[0];spec.build['container_command']=f'{pm if pm not in {"Unknown","npm"} else "npm"} run build' if build else ''
        if framework=='Astro':
            cf=_first(repo,{'astro.config.mjs','astro.config.js','astro.config.ts','astro.config.cjs'});cfg=repo.read(cf) if cf else '';deps={**pkg.get('dependencies',{}),**pkg.get('devDependencies',{})};adapter=next((n for d,n in [('@astrojs/vercel','vercel'),('@astrojs/node','node'),('@astrojs/netlify','netlify'),('@astrojs/cloudflare','cloudflare')] if d in deps or d in cfg),'unknown');output='server' if re.search(r"output\s*:\s*['\"]server['\"]",cfg) else ('hybrid' if re.search(r"output\s*:\s*['\"]hybrid['\"]",cfg) else 'static')
            ev=[x for x in (cf,'package.json') if x];check('FRAMEWORK_CONFIG','Framework adapter/output','pass' if cf else 'warning',ev,f'Astro adapter={adapter}, output={output}.')
        ...
        manifests=[x for x in ('requirements.txt','pyproject.toml','Pipfile') if x in repo.file_set];check('PYTHON_MANIFEST','Python dependency manifest','pass' if manifests else 'blocker',manifests,'Dependency manifest detected.' if manifests else 'No supported dependency manifest.')
        port=_port(repo.corpus,8000);fw=result['summary'].get('framework');start=None
        if fw=='Django':
        ...
        cs=next((f for f in repo.files if f.endswith('.csproj')),None);check('DOTNET_PROJECT','.NET project','pass' if cs else 'blocker',[cs] if cs else [],'Project file detected.' if cs else 'No .csproj found.');spec.build['runtime_strategy']='dotnet-publish' if cs else 'unknown';spec.processes[0]['start_command']='dotnet /app/app.dll';spec.network['port']=_port(repo.corpus,8080) if cs else spec.network.get('port',8080)
    elif primary=='PHP':
        ...
        if root or entry:spec.build['runtime_strategy']='php-nginx';spec.network['port']=80;spec.processes[0]['start_command']='nginx'
    elif primary=='Ruby':
