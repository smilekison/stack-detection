...
def dockerfile(spec):
    rt=spec.runtime.get('name','Unknown');fw=spec.frameworks[0]['name'] if spec.frameworks else '';port=spec.network.get('port') or 8000;strategy=spec.build.get('runtime_strategy');start=spec.processes[0].get('start_command','') if spec.processes else '';pm,setup,manifest,install=_node_install(spec)
    if rt=='Static Web':
        return '''FROM nginxinc/nginx-unprivileged:1.27-alpine\nCOPY --chown=nginx:nginx . /usr/share/nginx/html\nEXPOSE 8080\nCMD ["nginx", "-g", "daemon off;"]\n'''
    if rt=='Node.js':
        node=_tag(spec.runtime.get('version'),'20');build=spec.build.get('container_command') or 'npm run build'
        if strategy=='dev-server-fallback':return f'''FROM node:{node}-bookworm-slim AS build\nWORKDIR /app\n{setup}\n{manifest}\n{install}\nCOPY . .\nRUN {build}\nFROM node:{node}-bookworm-slim AS runtime\nWORKDIR /app\nRUN useradd --system --uid 10001 --no-create-home appuser\nCOPY --from=build --chown=10001:10001 /app /app\nUSER 10001\nENV HOST=0.0.0.0\nENV PORT={port}\nEXPOSE {port}\nCMD ["sh", "-c", {start or f'{pm} run dev -- --host 0.0.0.0 --port {port}'!r}]\n'''
        if strategy=='node-standalone':return f'''FROM node:{node}-bookworm-slim AS build\nWORKDIR /app\n{setup}\n{manifest}\n{install}\nCOPY . .\nRUN {build}\nFROM node:{node}-bookworm-slim AS runtime\nWORKDIR /app\nRUN useradd --system --uid 10001 --no-create-home appuser\nENV NODE_ENV=production HOST=0.0.0.0 PORT={port}\nCOPY --from=build --chown=10001:10001 /app/dist ./dist\nCOPY --from=build --chown=10001:10001 /app/node_modules ./node_modules\nUSER 10001\nEXPOSE {port}\nCMD ["node", "./dist/server/entry.mjs"]\n'''
        if strategy=='static-preview':return f'''FROM node:{node}-bookworm-slim AS build\nWORKDIR /app\n{setup}\n{manifest}\n{install}\nCOPY . .\nRUN {build}\nFROM node:{node}-bookworm-slim\nWORKDIR /app\nCOPY --from=build /app /app\nENV HOST=0.0.0.0 PORT={port}\nEXPOSE {port}\nCMD ["sh", "-c", "{pm} run preview -- --host 0.0.0.0 --port {port}"]\n'''
